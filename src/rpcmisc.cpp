...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total decentralshop balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in decentralshop/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in decentralshop/kb\n"
    ...
    Object zdecentralshopObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zdecentralshopObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zdecentralshopObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zDSCsupply", zdecentralshopObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"decentralshopaddress\"\n"
            "\nReturn information about the given decentralshop address.\n"
            "\nArguments:\n"
            "1. \"decentralshopaddress\"     (string, required) The decentralshop address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"decentralshopaddress\", (string) The decentralshop address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: DecentralShop address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are decentralshop addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) decentralshop address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"decentralshopaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"decentralshopaddress\"  (string, required) The decentralshop address to use for the signature.\n"
