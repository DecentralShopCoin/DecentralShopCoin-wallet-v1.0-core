...
#include "zdecentralshopcontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZDecentralShopControl_clicked()
{
    ZDecentralShopControlDialog* zDecentralShopControl = new ZDecentralShopControlDialog(this);
    zDecentralShopControl->setModel(walletModel);
    zDecentralShopControl->exec();
}

void PrivacyDialog::setZDecentralShopControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDecentralShopSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDSC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DecentralShop Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zDecentralShop selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDecentralShopControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDecentralShopControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdecentralshop selector in case it was used
    ZDecentralShopControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDecentralShop Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DecentralShop, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDecentralShop Mint");
