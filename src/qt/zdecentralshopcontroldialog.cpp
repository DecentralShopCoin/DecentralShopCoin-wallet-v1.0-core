...
#include "ui_zdecentralshopcontroldialog.h"
...
std::list<std::string> ZDecentralShopControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDecentralShopControlDialog::listMints;

ZDecentralShopControlDialog::ZDecentralShopControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDecentralShopControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDecentralShopControlDialog::~ZDecentralShopControlDialog()
{
    delete ui;
}

void ZDecentralShopControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDecentralShopControlDialog::updateList()
{
...
void ZDecentralShopControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDecentralShopControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDecentralShop_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDecentralShopControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDecentralShopControlDialog::GetSelectedMints()
{
...
void ZDecentralShopControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
